...
이미지 등 정적 데이터 조회를 위해서는 아래 도메인에 리소스 경로를 붙여 요청합니다.
...
```http
PATCH /travel-rooms/{travelRoomId}
```
...
| 이름      | 타입      | 설명           | 제약 조건              | 필수 여부 |
| --------- | --------- | -------------- | ---------------------- | --------- |
| name      | string    | 여행방 이름    |                        | 선택      |
| startDate | date?     | 여행 시작 일시 |                        | 선택      |
| endDate   | date?     | 여행 종료 일시 |                        | 선택      |
| countries | Country[] | 국가 리스트    | 배열 요소 갯수는 0이상 | 선택      |
...
```http
POST /travel-rooms/{travelRoomId}/leave
```
...
| 파라미터     | 설명                 |
| ------------ | -------------------- |
| travelRoomId | 나갈 여행방의 아이디 |

#### 응답 바디

없음

### 여행방 커버 이미지 업로드를 위한 url 발급받기
